...
# smolvla_bridge_cmp_node 래퍼 스크립트 경로 (Where-collapse 비교실험 cond A/B/C/D)
# (venv_SmolVLA310 Python + CUDA lib + ROS2 pypath 설정 후 노드 실행)
_BRIDGE_DIRECT_SH = os.path.expanduser(
    "~/SmolVLA/SmolVLA-INFERENCE/scripts/run_bridge_cmp_node.sh"
)
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            "model_path",
            # cmp 체크포인트 경로 (cond_A~D 의 pretrained_model, 또는 _merged).
            # PEFT 어댑터 디렉토리를 그대로 줘도 노드가 base+adapter로 직접 로드함.
            # 예) ros2 launch ... model_path:=/media/billye6/.../cond_B_lora_256/checkpoints/last/pretrained_model
            default_value=(
        ...
        # 비우면 저장 안 함. 경로를 주면 모델 입력(256) 이미지를 그 아래
        # <cond태그>_<타임스탬프>/ 폴더에 매 추론마다 저장.
        # "none"이면 저장 안 함(기본). 경로를 주면 그 아래 <cond태그>_<타임스탬프>/에 저장.
        # (빈 문자열은 rcl 파라미터 파서가 거부하므로 sentinel "none" 사용)
        DeclareLaunchArgument("save_image_dir", default_value="none"),
        # true면 크롭 없이 raw 전체를 바로 512×512로 리사이즈해서 모델에 입력.
        # false(기본)면 기존 크롭→224 파이프라인 그대로.
        DeclareLaunchArgument("no_crop", default_value="false"),
        ...
        # cmp(v4) 데이터셋은 6-phase prompt로 학습됨 → per_frame_v16이 학습과 일치하는 기본값.
        DeclareLaunchArgument("prompt_mode", default_value="per_frame_v16"),
        DeclareLaunchArgument("source_side", default_value="left"),
        DeclareLaunchArgument("target_side", default_value="right"),
        # per_frame_v16 물체 명사 (기본 "orange box"; 예: object_name:="red block")
        DeclareLaunchArgument("object_name", default_value="orange box"),
        # per_frame_v16 방향 문구 제거 (true면 "...on the left side" 등 접미사 삭제)
        DeclareLaunchArgument("omit_direction", default_value="false"),
        ...
        Node(
            ...
            parameters=[{
                ...
                "no_crop": LaunchConfiguration("no_crop"),
            }],
        ),

        # SmolVLA cmp(A/B/C/D) 직접 추론 브릿지 (HTTP 서버 불필요)
        # venv_SmolVLA310 Python으로 실행해야 CUDA torch + lerobot + peft가 동작함.
        ...
        ExecuteProcess(
            cmd=[
                ...
                _BRIDGE_DIRECT_SH,
                "--ros-args",
                "-r", "__node:=smolvla_bridge_cmp_node",
        ...
        Node(
            ...
            parameters=[{
                ...
                "omit_direction": LaunchConfiguration("omit_direction"),

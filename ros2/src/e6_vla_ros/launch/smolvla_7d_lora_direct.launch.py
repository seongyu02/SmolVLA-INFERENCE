...
# smolvla_bridge_7d_lora_node 래퍼 스크립트 경로
# (venv_SmolVLA310 Python + CUDA lib + ROS2 pypath 설정 후 노드 실행)
_BRIDGE_DIRECT_SH = os.path.expanduser(
    "~/SmolVLA/SmolVLA-INFERENCE/scripts/run_bridge_7d_lora_node.sh"
)
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            "model_path",
            # 원격에서 LoRA를 base에 merge한 full checkpoint 경로. 복사한 실제 경로로 덮어쓰면 됨.
            # 예) ros2 launch ... model_path:=/media/billye6/.../checkpoints/030000/pretrained_model
            default_value=(
        ...
        # v4 데이터셋은 6-phase prompt로 학습됨 → per_frame_v16이 학습과 일치하는 기본값.
        ...
        # SmolVLA 7D LoRA 직접 추론 브릿지 (HTTP 서버 불필요)
        ...
        ExecuteProcess(
            cmd=[
                ...
                _BRIDGE_DIRECT_SH,
                "--ros-args",
                "-r", "__node:=smolvla_bridge_7d_lora_node",

#!/usr/bin/env python3
"""
SmolVLA 7D direct inference bridge node.
...
class SmolVLABridgeDirectNode(Node):
    def __init__(self):
        super().__init__("smolvla_bridge_direct_node")
        ...
        self.get_logger().info(
            f"[direct] bridge started model={self._model_path} "
    ...
    def _load_model(self):
        self.get_logger().info(f"[direct] loading model: {self._model_path}")
        t0 = time.monotonic()
        try:
    ...
    def _cb_prompt(self, msg: String):
        ...
        if changed:
            self.get_logger().info(f"[direct] prompt changed: {msg.data!r}")
    ...
    def _cb_task_status(self, msg: String):
        if not self._task_complete and (
            ...
            self.get_logger().info(f"[direct] inference stopped ({msg.data})")
    ...
    def _maybe_infer(self):
        if (
            ...
            if not self._model_ready:
                self.get_logger().info(
                    "[direct] waiting for model to load...",
        ...
        if obs_state.shape[0] != STATE_DIM:
            self.get_logger().warn(
                f"[direct] state len={obs_state.shape[0]} expected {STATE_DIM}",
                throttle_duration_sec=5.0,
            )
            return
        ...
        if self._init_pose_armed:
            ...
            if j_diff > self._init_pose_tol:
                self.get_logger().info(
                    f"[direct] waiting for init pose "
            ...
            self.get_logger().info(
                f"[direct] init pose confirmed, starting inference "
    ...
    def _run_infer(self, obs: dict):
        ...
        try:
            ...
            self.get_logger().info(
                f"[direct] inference {total_ms:.0f}ms "
                f"(preproc={preproc_ms:.0f}ms predict={predict_ms:.0f}ms "
                f"postproc={postproc_ms:.0f}ms) shape={actions_np.shape}"
            )
            self.get_logger().info(
                f"[direct] state7={np.round(obs['state7'], 1).tolist()}"
            )
            self.get_logger().info(
                f"[direct] action_7={np.round(actions_np[0], 3).tolist()}"
            )
            self.get_logger().info(f"[direct] suction_seq: {grip_vals}")

            self._chunk_pub.publish(Float32MultiArray(data=actions_np.flatten().tolist()))

        except Exception as exc:
            self.get_logger().error(f"[direct] inference failed: {exc}")
        finally:
            self._inference_running = False

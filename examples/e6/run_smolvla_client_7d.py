#!/usr/bin/env python3
"""
SmolVLA Dobot E6 7D 추론 클라이언트 — HTTP 정책 서버 연결 버전

run_e6_client.py와 동일한 구조/FSM/제어 로직.
openpi WebsocketClientPolicy 대신 HTTP POST /act 로 SmolVLA 7D 서버에 연결.

아키텍처:
  [serve_policy_smolvla_7d.py] ← HTTP POST /act ← [이 스크립트] → Dobot E6
...
액션 계약 (SmolVLA 7D):
  ...
  action[6]    흡착 절대값 (0/1, 연속값 → hysteresis 처리)

서버 실행 예 (Terminal 1):
  cd ~/SmolVLA/SmolVLA-INFERENCE && bash scripts/run_server_7d_expert.sh

클라이언트 실행 예 (Terminal 2):
  cd ~/SmolVLA/SmolVLA-INFERENCE && bash scripts/run_client_7d_expert.sh
"""
...
def main() -> None:
    ...
    parser.add_argument("--server_port", type=int, default=8001)
    ...
    # ── 제어 루프 ─────────────────────────────────────────────────────────────
    parser.add_argument("--hz", type=float, default=16.0, help="제어 주파수")
    ...
    try:
        while True:
            ...
            # ── 액션 파싱 (SmolVLA 7D → delta joints + gripper) ──────────────
